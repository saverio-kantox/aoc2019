defmodule Day02 do
  ...
  * 1,1,1,4,99,5,6,0,99 becomes 30,1,1,4,2,5,6,0,99.
  ...
  iex> "priv/day_02_input.txt" |> File.read!() |> Day02.one()
  5290681

  iex> "priv/day_02_input.txt" |> File.read!() |> Day02.two(19_690_720)
  5741
  """
  ...
  def one(inputs) do
    inputs
    |> Aoc.Intcode.new()
    |> Aoc.Intcode.configure([{1, 12}, {2, 2}])
    |> Aoc.Intcode.run()
    |> Aoc.Intcode.read(0)
  end

  @spec two(binary, non_neg_integer()) :: non_neg_integer
  def two(inputs, expected) do
    {n, v, _} =
      Stream.iterate(0, &(&1 + 1))
      |> Stream.map(
        &(Enum.map(0..&1, fn x -> [{&1, x}, {x, &1}] end)
          |> List.flatten()
          |> Enum.uniq())
      )
      |> Stream.concat()
      |> Stream.map(fn {noun, verb} ->
        {noun, verb,
         inputs
         |> Aoc.Intcode.new()
         |> Aoc.Intcode.configure([{1, noun}, {2, verb}])
         |> Aoc.Intcode.run()
         |> Aoc.Intcode.read(0)}
      end)
      |> Stream.filter(&(elem(&1, 2) == expected))
      |> Enum.take(1)
      |> hd()
